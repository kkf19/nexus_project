...
def walk(node, path, ctx, errors, stats):
    if isinstance(node, dict):
        ...
        if layer is not None:
            ...
            # V11 — verification status (Tier 2 H)
            vs = node.get("verification_status")
            if vs is not None and vs not in ("reported", "validated", "verified"):
                errors.append(("V11", path, f"invalid verification_status {vs}"))
            if layer == OFF and vs not in (None, "reported"):
                errors.append(("V11", path, "OFFICIAL_JCI_FACT cannot be validated/verified by the engine: the report describes no verification"))
            if vs == "verified" and not node.get("evidence_ref"):
                errors.append(("V11", path, "verified without evidence_ref"))
